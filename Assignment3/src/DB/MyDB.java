...
public class MyDB {
	...
	public void updateProduct(Product prod) {
		...
		String message = "UPDATE IGNORE Product set description = \""
				...
				+ prod.getProductNumber() + " where id = "
				+ Product.getIDNumber() + ", version = " + prod.getVersion();
	...
	public User login(String user, String pw) {
		String message = "SELECT * FROM Login where user = \"" + user + "\"";
		String str;
		try {
			Statement s = connect.createStatement();
			ResultSet r = s.executeQuery(message);
			...
			if (r.next()) {
				str = r.getString("pw");
				if (str.equals(pw)) {
					return new User(r.getString("user"), r.getString("email"),
							r.getInt("permissions"));
				}
			}
		} catch (SQLException e) {
			e.printStackTrace();
